...
const months = {
  ...
  GE_LOCATIVE: ["იანვარში","თებერვალში","მარტში","აპრილში","მაისში","ივნისში","ივლისში","აგვისტოში","სექტემბერში","ოქტომბერში","ნოემბერში","დეკემბერში"],
...
const LeftPanel = ({ language }) => {
  ...
  return (
    <div className="w-full px-4 py-6 space-y-6">
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="flex items-center justify-center gap-2">
          <h1 className="text-lg sm:text-xl text-gray-800 font-bpg-nino text-center whitespace-nowrap">
            {language === "GE" ? "გაანგარიშების ინსტრუქცია" : "CALCULATION INSTRUCTION"}
          </h1>
          <button onClick={() => setIsModalOpen(true)} className="text-black hover:text-gray-600 transition-colors cursor-pointer" aria-label="Information">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <select value={month} onChange={(e) => setMonth(e.target.value)} className="border border-gray-300 rounded-md px-3 py-2 text-sm font-bpg-nino w-full">
          ...
          {months[language === "GE" ? "GE" : "EN"].map((m, i) => {
            ...
            return <option key={i} value={monthValue} disabled={isCurrentYear && monthValue > prevMonth}>{m}</option>;
          })}
        </select>
      ...
      {isModalOpen && <InfoModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} language={language} />}
    </div>
  );
};
